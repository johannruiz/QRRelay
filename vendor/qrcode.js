// Vendored QR encoder derived from Kazuhiko Arase's QRCode for JavaScript.
// Local ESM bundle generated for QR Relay; see THIRD_PARTY_NOTICES.md.
...
QRCode.prototype = {
	...
	make : function() {
		...
		if (this.typeNumber < 1 ){
			...
			if (typeNumber > 40) throw new Error("QR data exceeds version 40 capacity");
			this.typeNumber = typeNumber;
		}
...
function __load(id) {
  const normalized = id.replace(/^\.\//u, "").replace(/\.js$/u, "");
  ...
  factory(module, module.exports, __load);
  return module.exports;
}
const QRCode = __load("index");
const ErrorCorrectLevel = __load("QRErrorCorrectLevel");

/** Return a compact byte-per-module QR matrix for an ASCII string. */
export function createQrMatrix(text, level = "M") {
  if (typeof text !== "string" || !/^[\x20-\x7e]*$/u.test(text)) {
    throw new Error("QR encoder accepts printable ASCII only.");
  }
  const correction = ErrorCorrectLevel[level] ?? ErrorCorrectLevel.M;
  ...
  for (let row = 0; row < size; row += 1) {
    for (let column = 0; column < size; column += 1) {
      modules[row * size + column] = qr.isDark(row, column) ? 1 : 0;
    }
  }
  return { size, modules };
}
